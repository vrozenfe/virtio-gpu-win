...
VioGpu3D::VioGpu3D(_In_ DEVICE_OBJECT* pPhysicalDeviceObject) : m_pPhysicalDevice(pPhysicalDeviceObject),
...
BOOLEAN VioGpu3D::CheckHardware()
{
    PAGED_CODE();
...
NTSTATUS VioGpu3D::StartDevice(_In_  DXGK_START_INFO*   pDxgkStartInfo,
                         ...
                         _Out_ ULONG*             pNumberOfChildren)
{
    ...
    m_pAdapter = new(NonPagedPoolNx) VioGpuAdapter(this);
    if (!m_pAdapter)
    {
    ...
    Status = m_pAdapter->HWInit(m_DeviceInfo.TranslatedResourceList, &m_CurrentModes[0].DispInfo);
    if (!NT_SUCCESS(Status))
    {
    ...
    //Status = RegisterHWInfo(m_pAdapter->GetId());
    //if (!NT_SUCCESS(Status))
    //{
...
NTSTATUS VioGpu3D::StopDevice(VOID)
{
    PAGED_CODE();
...
VOID VioGpu3D::CleanUp(VOID)
{
    ...
    //    {
    //        m_pAdapter->ReleaseFrameBuffer(&m_CurrentModes[Source]);
    //    }
    //}
}


NTSTATUS VioGpu3D::DispatchIoRequest(_In_  ULONG VidPnSourceId,
...
NTSTATUS VioGpu3D::SetPowerState(_In_  ULONG HardwareUid,
                               ...
                               _In_  POWER_ACTION       ActionType)
{
    ...
    {
        Status = m_pAdapter->SetPowerState(DevicePowerState, &(m_CurrentModes[0].DispInfo));
...
NTSTATUS VioGpu3D::QueryChildRelations(_Out_writes_bytes_(ChildRelationsSize) DXGK_CHILD_DESCRIPTOR* pChildRelations,
...
NTSTATUS VioGpu3D::QueryChildStatus(_Inout_ DXGK_CHILD_STATUS* pChildStatus,
...
// EDID retrieval
NTSTATUS VioGpu3D::QueryDeviceDescriptor(_In_    ULONG                   ChildUid,
...
NTSTATUS VioGpu3D::QueryAdapterInfo(_In_ CONST DXGKARG_QUERYADAPTERINFO* pQueryAdapterInfo)
{
    PAGED_CODE();
    ...
    switch (pQueryAdapterInfo->Type)
    {
        case DXGKQAITYPE_UMDRIVERPRIVATE:
        {
            ...
            m_pAdapter->FillPrivateData(pQueryAdapterInfo->pOutputData);
        }
        break;
        case DXGKQAITYPE_DRIVERCAPS:
        {
            ...
            pDriverCaps->MaxAllocationListSlotId = (UINT)-1;//32;

            if (m_pAdapter->EnablePointer()) {
...
//            pDriverCaps->FlipCaps.FlipOnVSyncWithNoWait = 1;
            ...
            pDriverCaps->WDDMVersion = DXGKDDI_WDDMv1; //PFM!!!!!
            pDriverCaps->PresentationCaps.NoScreenToScreenBlt = 0;
            pDriverCaps->PresentationCaps.NoOverlapScreenBlt = 0;
            pDriverCaps->SchedulingCaps.MultiEngineAware = 0;
        ...
        case DXGKQAITYPE_QUERYSEGMENT:
        {
            ...
            m_pAdapter->QuerySegment(pQuerySegmentOut);
        }
        break;
        default:
        {
...
NTSTATUS VioGpu3D::SetPointerPosition(_In_ CONST DXGKARG_SETPOINTERPOSITION* pSetPointerPosition)
{
    ...
    return m_pAdapter->SetPointerPosition(pSetPointerPosition, &m_CurrentModes[pSetPointerPosition->VidPnSourceId]);
}

NTSTATUS VioGpu3D::SetPointerShape(_In_ CONST DXGKARG_SETPOINTERSHAPE* pSetPointerShape)
{
    PAGED_CODE();
    ...
    return m_pAdapter->SetPointerShape(pSetPointerShape, &m_CurrentModes[pSetPointerShape->VidPnSourceId]);
}

NTSTATUS VioGpu3D::PresentDisplayOnly(_In_ CONST DXGKARG_PRESENT_DISPLAYONLY* pPresentDisplayOnly)
{
    PAGED_CODE();
    ...
    Status = m_pAdapter->ExecutePresentDisplayOnly(
                        pDst,
                        DstBitPerPixel,
...
NTSTATUS VioGpu3D::QueryInterface(_In_ CONST PQUERY_INTERFACE pQueryInterface)
{
    PAGED_CODE();
                                                          ...
                                                          _Out_ DXGK_DISPLAY_INFORMATION*      pDisplayInfo)
{
    ...
    m_pAdapter->BlackOutScreen(&m_CurrentModes[SourceId]);
...
NTSTATUS VioGpu3D::QueryVidPnHWCapability(_Inout_ DXGKARG_QUERYVIDPNHWCAPABILITY* pVidPnHWCaps)
{
    PAGED_CODE();
...
NTSTATUS VioGpu3D::IsSupportedVidPn(_Inout_ DXGKARG_ISSUPPORTEDVIDPN* pIsSupportedVidPn)
{
    PAGED_CODE();
...
NTSTATUS VioGpu3D::RecommendFunctionalVidPn(_In_ CONST DXGKARG_RECOMMENDFUNCTIONALVIDPN* CONST pRecommendFunctionalVidPn)
{
    PAGED_CODE();
...
NTSTATUS VioGpu3D::RecommendVidPnTopology(_In_ CONST DXGKARG_RECOMMENDVIDPNTOPOLOGY* CONST pRecommendVidPnTopology)
{
    PAGED_CODE();
...
NTSTATUS VioGpu3D::RecommendMonitorModes(_In_ CONST DXGKARG_RECOMMENDMONITORMODES* CONST pRecommendMonitorModes)
{
    PAGED_CODE();
...
NTSTATUS VioGpu3D::AddSingleSourceMode(_In_ CONST DXGK_VIDPNSOURCEMODESET_INTERFACE* pVidPnSourceModeSetInterface,
                                                   ...
                                                   D3DDDI_VIDEO_PRESENT_SOURCE_ID SourceId)
{
    ...
    for (ULONG idx = 0; idx < m_pAdapter->GetModeCount(); ++idx)
    {
        // Create new mode info that will be populated
        D3DKMDT_VIDPN_SOURCE_MODE* pVidPnSourceModeInfo = NULL;
        PVIDEO_MODE_INFORMATION pModeInfo = m_pAdapter->GetModeInfo(idx);
        ...
        if (!NT_SUCCESS(Status))
        {
            ...
            {
                DbgPrint(TRACE_LEVEL_ERROR, ("%s pfnAddMode failed with Status = 0x%X, hVidPnSourceModeSet = 0x%I64x, pVidPnSourceModeInfo = %p\n",
                    __FUNCTION__, Status, hVidPnSourceModeSet, pVidPnSourceModeInfo));
                return Status;
            }
        }
    }
...
NTSTATUS VioGpu3D::AddSingleTargetMode(_In_ CONST DXGK_VIDPNTARGETMODESET_INTERFACE* pVidPnTargetModeSetInterface,
                                                   ...
                                                   D3DDDI_VIDEO_PRESENT_SOURCE_ID SourceId)
{
    ...
    for (UINT ModeIndex = 0; ModeIndex < m_pAdapter->GetModeCount(); ++ModeIndex)
    {
        PVIDEO_MODE_INFORMATION pModeInfo = m_pAdapter->GetModeInfo(SourceId);
        ...
        pVidPnTargetModeInfo->VideoSignalInfo.VSyncFreq.Numerator = m_pAdapter->GetVSyncFreq() * 1000;
        pVidPnTargetModeInfo->VideoSignalInfo.VSyncFreq.Denominator = 1000;
        pVidPnTargetModeInfo->VideoSignalInfo.HSyncFreq.Numerator = m_pAdapter->GetVSyncFreq() * pModeInfo->VisScreenHeight * 1000 * (105 / 100);;
        pVidPnTargetModeInfo->VideoSignalInfo.HSyncFreq.Denominator = 1000;
        pVidPnTargetModeInfo->VideoSignalInfo.PixelRate = m_pAdapter->GetVSyncFreq() * pModeInfo->VisScreenWidth * pModeInfo->VisScreenHeight;
    ...
    // We add this as PREFERRED since it is the only supported target
        ...
        if (!NT_SUCCESS(Status))
        {
            if (Status != STATUS_GRAPHICS_MODE_ALREADY_IN_MODESET)
            {
                DbgPrint(TRACE_LEVEL_ERROR, ("%s pfnAddMode failed with Status = 0x%X, hVidPnTargetModeSet = 0x%I64x, pVidPnTargetModeInfo = %p\n",
                    __FUNCTION__, Status, hVidPnTargetModeSet, pVidPnTargetModeInfo));
            }
...
NTSTATUS VioGpu3D::AddSingleMonitorMode(_In_ CONST DXGKARG_RECOMMENDMONITORMODES* CONST pRecommendMonitorModes)
{
    PAGED_CODE();
    ...
    pVbeModeInfo = m_pAdapter->GetModeInfo(m_pAdapter->GetCurrentModeIndex());
    ...
    pMonitorSourceMode->VideoSignalInfo.VSyncFreq.Numerator = m_pAdapter->GetVSyncFreq() * 1000;
    pMonitorSourceMode->VideoSignalInfo.VSyncFreq.Denominator = 1000;
    pMonitorSourceMode->VideoSignalInfo.HSyncFreq.Numerator = m_pAdapter->GetVSyncFreq() * pVbeModeInfo->VisScreenHeight * 1000 * (105 / 100);;
    pMonitorSourceMode->VideoSignalInfo.HSyncFreq.Denominator = 1000;
    pMonitorSourceMode->VideoSignalInfo.PixelRate = m_pAdapter->GetVSyncFreq() * pVbeModeInfo->VisScreenWidth * pVbeModeInfo->VisScreenHeight;
    ...
    pMonitorSourceMode->Preference = D3DKMDT_MP_NOTPREFERRED; //Until we support EDID
    ...
    pMonitorSourceMode->ColorCoeffDynamicRanges.FourthChannel = 8; //0; ??
    ...
    if (!NT_SUCCESS(Status))
    {
        if (Status != STATUS_GRAPHICS_MODE_ALREADY_IN_MODESET)
        {
            DbgPrint(TRACE_LEVEL_ERROR, ("%s pfnAddMode failed with Status = 0x%X, hMonitorSourceModeSet = 0x%I64x, pMonitorSourceMode = 0x%I64x\n",
                __FUNCTION__, Status, pRecommendMonitorModes->hMonitorSourceModeSet, pMonitorSourceMode));
        }
        else
        {
    ...
    for (UINT Idx = 0; Idx < m_pAdapter->GetModeCount(); ++Idx)
    {
        // There is only one source format supported by display-only drivers, but more can be added in a 
        // full WDDM driver if the hardware supports them

        pVbeModeInfo = m_pAdapter->GetModeInfo(Idx);
        ...
        if (!NT_SUCCESS(Status))
        {
            if (Status != STATUS_GRAPHICS_MODE_ALREADY_IN_MODESET)
            {
                DbgPrint(TRACE_LEVEL_ERROR, ("%s pfnAddMode failed with Status = 0x%X, hMonitorSourceModeSet = 0x%I64x, pMonitorSourceMode = 0x%p",
                                __FUNCTION__, Status, pRecommendMonitorModes->hMonitorSourceModeSet, pMonitorSourceMode));
            }
...
NTSTATUS VioGpu3D::EnumVidPnCofuncModality(_In_ CONST DXGKARG_ENUMVIDPNCOFUNCMODALITY* CONST pEnumCofuncModality)
{
    PAGED_CODE();
...
NTSTATUS VioGpu3D::SetVidPnSourceVisibility(_In_ CONST DXGKARG_SETVIDPNSOURCEVISIBILITY* pSetVidPnSourceVisibility)
{
    PAGED_CODE();
...
NTSTATUS VioGpu3D::CommitVidPn(_In_ CONST DXGKARG_COMMITVIDPN* CONST pCommitVidPn)
{
    PAGED_CODE();
    ...
    {
        Status = m_pAdapter->ReleaseFrameBuffer(&m_CurrentModes[pCommitVidPn->AffectedVidPnSourceId]);
        if (!NT_SUCCESS(Status))
        {
            goto CommitVidPnExit;
        }
...
NTSTATUS VioGpu3D::SetSourceModeAndPath(CONST D3DKMDT_VIDPN_SOURCE_MODE* pSourceMode,
                                         CONST D3DKMDT_VIDPN_PRESENT_PATH* pPath)
{
    ...
    Status = m_pAdapter->AcquireFrameBuffer(pCurrentBddMode);

    if (NT_SUCCESS(Status))
    {
        pCurrentBddMode->Flags.FullscreenPresent = TRUE;
        for (USHORT ModeIndex = 0; ModeIndex < m_pAdapter->GetModeCount(); ++ModeIndex)
        {
             PVIDEO_MODE_INFORMATION pModeInfo = m_pAdapter->GetModeInfo(ModeIndex);
             if (pCurrentBddMode->DispInfo.Width == pModeInfo->VisScreenWidth &&
                 pCurrentBddMode->DispInfo.Height == pModeInfo->VisScreenHeight )
             {
                 Status = m_pAdapter->SetCurrentMode(m_pAdapter->GetModeNumber(ModeIndex), pCurrentBddMode);
                 if (NT_SUCCESS(Status))
                 {
                     m_pAdapter->SetCurrentModeIndex(ModeIndex);
                 }
                 break;
             }
        }
...
NTSTATUS VioGpu3D::IsVidPnPathFieldsValid(CONST D3DKMDT_VIDPN_PRESENT_PATH* pPath) const
{
    PAGED_CODE();
...
NTSTATUS VioGpu3D::IsVidPnSourceModeFieldsValid(CONST D3DKMDT_VIDPN_SOURCE_MODE* pSourceMode) const
{
    PAGED_CODE();
...
NTSTATUS VioGpu3D::UpdateActiveVidPnPresentPath(_In_ CONST DXGKARG_UPDATEACTIVEVIDPNPRESENTPATH* CONST pUpdateActiveVidPnPresentPath)
{
    PAGED_CODE();
...
NTSTATUS VioGpu3D::DestroyAllocation(_In_ CONST DXGKARG_DESTROYALLOCATION *pDestroyAllocation)
{
    PAGED_CODE();
    NTSTATUS Status = STATUS_SUCCESS;
    ...
    DbgPrint(TRACE_LEVEL_VERBOSE, ("---> %s\n", __FUNCTION__));
//    Status = m_pAdapter->DestroyAllocation(pDestroyAllocation);
    DbgPrint(TRACE_LEVEL_VERBOSE, ("<--- %s\n", __FUNCTION__));

    return Status;
}

NTSTATUS VioGpu3D::CreateAllocation(_In_ DXGKARG_CREATEALLOCATION *pCreateAllocation)
{
    PAGED_CODE();
    NTSTATUS Status = STATUS_SUCCESS;
    DXGK_ALLOCATIONINFO *pAllocInfo = NULL;
    DbgPrint(TRACE_LEVEL_VERBOSE, ("---> %s\n", __FUNCTION__));

    pAllocInfo = pCreateAllocation->pAllocationInfo;

    // Not using outer pPrivateDriverData struct, loop and fill in allocations.
    //for (UINT i = 0; i < pCreateAllocation->NumAllocations; i++, pAllocInfo++)
    //{
    //    pDriverAllocation = 
...
VOID VioGpu3D::DpcRoutine(VOID)
{
    DbgPrint(TRACE_LEVEL_VERBOSE, ("---> %s\n", __FUNCTION__));
    m_pAdapter->DpcRoutine(&m_DxgkInterface);
    m_DxgkInterface.DxgkCbNotifyDpc((HANDLE)m_DxgkInterface.DeviceHandle);
    DbgPrint(TRACE_LEVEL_VERBOSE, ("<--- %s\n", __FUNCTION__));
}

BOOLEAN VioGpu3D::InterruptRoutine(_In_  ULONG MessageNumber)
{
    DbgPrint(TRACE_LEVEL_VERBOSE, ("<--> %s\n", __FUNCTION__));
    if(IsHardwareInit()) {
        return m_pAdapter ? m_pAdapter->InterruptRoutine(&m_DxgkInterface, MessageNumber) : FALSE;
    }
    return FALSE;
}

VOID VioGpu3D::ResetDevice(VOID)
{
    DbgPrint(TRACE_LEVEL_VERBOSE, ("<---> %s\n", __FUNCTION__));
    m_pAdapter->ResetDevice();
}

// Must be Non-Paged, as it sets up the display for a bugcheck
NTSTATUS VioGpu3D::SystemDisplayEnable(_In_  D3DDDI_VIDEO_PRESENT_TARGET_ID TargetId,
...
VOID VioGpu3D::SystemDisplayWrite(_In_reads_bytes_(SourceHeight * SourceStride) VOID* pSource,
...
NTSTATUS VioGpu3D::WriteHWInfoStr(_In_ HANDLE DevInstRegKeyHandle, _In_ PCWSTR pszwValueName, _In_ PCSTR pszValue)
{
    PAGED_CODE();
...
NTSTATUS VioGpu3D::RegisterHWInfo(_In_ ULONG Id)
{
    PAGED_CODE();
    ...
    UNICODE_STRING VioVioGpuAdapterID;
    RtlInitUnicodeString(&VioVioGpuAdapterID, L"VioVioGpuAdapterID");
...
#pragma code_seg(push)
#pragma code_seg()
D3DDDI_VIDEO_PRESENT_SOURCE_ID VioGpu3D::FindSourceForTarget(D3DDDI_VIDEO_PRESENT_TARGET_ID TargetId, BOOLEAN DefaultToZero)
{
...
VioGpuAdapter::VioGpuAdapter(_In_ VioGpu3D* pVioGpu3D)
{
    PAGED_CODE();
...
NTSTATUS VioGpuAdapter::VioGpuAdapterInit(DXGK_DISPLAY_INFORMATION* pDispInfo)
{
    ...
    if (status == STATUS_SUCCESS)
    {
       ...
       m_pVioGpu3D->SetHardwareInit(TRUE);
    }
    else
    {
...
void VioGpuAdapter::VioGpuAdapterClose()
{
    PAGED_CODE();
    DbgPrint(TRACE_LEVEL_VERBOSE, ("---> %s\n", __FUNCTION__));
    m_pVioGpu3D->SetHardwareInit(FALSE);
...
NTSTATUS VioGpuAdapter::SetPointerShape(_In_ CONST DXGKARG_SETPOINTERSHAPE* pSetPointerShape, _In_ CONST CURRENT_BDD_MODE* pModeCur)
{
    ...
    DestroyCursorObj();
    if (CreateCursorObj(pSetPointerShape, pModeCur))
    {
        ...
        crsr->resource_id = m_pCursorObj->GetId();
        crsr->pos.x = 0;
        crsr->pos.y = 0;
...
VOID VioGpuAdapter::FillPrivateData(_Out_ PVOID pData)
{
	PAGED_CODE();
...
NTSTATUS VioGpuAdapter::UpdateChildStatus(BOOLEAN connect)
{
    ...
    PDXGKRNL_INTERFACE pDXGKInterface(m_pVioGpu3D->GetDxgkInterface());
...
VOID VioGpuAdapter::QuerySegment(_Out_ DXGK_QUERYSEGMENTOUT* pQuerySegmentOut)
{
    pQuerySegmentOut->pSegmentDescriptor[0].BaseAddress = m_FrameSegment.GetPhysicalAddress();
    pQuerySegmentOut->pSegmentDescriptor[0].CpuTranslatedAddress = m_FrameSegment.GetPhysicalAddress();
    pQuerySegmentOut->pSegmentDescriptor[0].Size = m_FrameSegment.GetSize();
    pQuerySegmentOut->pSegmentDescriptor[0].CommitLimit = m_FrameSegment.GetSize();
    ...
    pQuerySegmentOut->pSegmentDescriptor[0].Flags.PopulatedFromSystemMemory = m_FrameSegment.IsSystemMemory();
...
void VioGpuAdapter::CreateFrameBufferObj(PVIDEO_MODE_INFORMATION pModeInfo, CURRENT_BDD_MODE* pCurrentBddMode)
{
    ...
    m_pFrameObj = new(NonPagedPoolNx) VioGpuObj();
    if (!m_pFrameObj->Init(size, &m_FrameSegment))
    {
    ...
    GpuObjectAttach(resid, m_pFrameObj);
    ...
    pCurrentBddMode->DispInfo.PhysicAddress = m_pFrameObj->GetPhysicalAddress();
    pCurrentBddMode->FrameBuffer.Ptr = m_pFrameObj->GetVirtualAddress();
...
BOOLEAN VioGpuAdapter::CreateCursorObj(_In_ CONST DXGKARG_SETPOINTERSHAPE* pSetPointerShape, _In_ CONST CURRENT_BDD_MODE* pCurrentBddMode)
{
    UINT resid, format, size;
    PAGED_CODE();
    ...
    m_pCursorObj = new(NonPagedPoolNx) VioGpuObj();
    if (!m_pCursorObj->Init(size, &m_CursorSegment))
    {
        DbgPrint(TRACE_LEVEL_FATAL, ("<--- %s Failed to init obj size = %d\n", __FUNCTION__, size));
        delete m_pCursorObj;
        m_pCursorObj = NULL;
    ...
    if (!GpuObjectAttach(resid, m_pCursorObj))
    {
    ...
    BLT_INFO DstBltInfo;
    DstBltInfo.pBits = m_pCursorObj->GetVirtualAddress();
...
void VioGpuAdapter::DestroyCursorObj()
{
    PAGED_CODE();
    DbgPrint(TRACE_LEVEL_VERBOSE, ("---> %s\n", __FUNCTION__));
    if (m_pCursorObj != NULL)
    {
        UINT id = m_pCursorObj->GetId();
        m_CtrlQueue.InvalBacking(id);
        m_CtrlQueue.UnrefResource(id);
        delete m_pCursorObj;
        m_pCursorObj = NULL;
...
)
{
    ...
    m_hDevice = pCreateDevice->hDevice;
    m_pAdapter = (VioGpuAdapter*)hAdapter;
    pCreateDevice->pInfo = NULL;
    DbgPrint(TRACE_LEVEL_VERBOSE, ("<---> %s SystemDevice = %d GdiDevice = %d\n", __FUNCTION__, m_Flags.SystemDevice, m_Flags.GdiDevice));
}
...
NTSTATUS
VioGpuDevice::CloseAllocation(
    _In_ HANDLE                     hDevice,
    _In_ DXGKARG_CLOSEALLOCATION*   pCloseAlloc
)
{
    UNREFERENCED_PARAMETER(hDevice);
    UNREFERENCED_PARAMETER(pCloseAlloc);
    return STATUS_SUCCESS;
}

NTSTATUS
VioGpuDevice::OpenAllocation(
    _In_ DXGKARG_OPENALLOCATION* pOpenAlloc
)
{
    UNREFERENCED_PARAMETER(pOpenAlloc);
    return STATUS_SUCCESS;
}
